...
export function AccountCard({ account, showAccountId = false, onAction }: AccountCardProps) {
  ...
  const idSuffix = showAccountId ? ` | Internal ${compactId}` : "";
    ...
    <div className="card-hover rounded-xl border bg-card p-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {!emailSubtitle && showUpstreamId ? (
            <p className="mt-0.5 truncate text-xs text-muted-foreground" title={`Upstream ${account.upstreamAccountId}`}>
              Upstream {compactUpstreamId}
            </p>
          ) : null}
          {emailSubtitle ? (
            <p
            ...
            >
              ...
              {showUpstreamId || showAccountId ? ` | ${[
                showUpstreamId ? `Upstream ${compactUpstreamId}` : null,
                showAccountId ? `Internal ${compactId}` : null,
              ].filter(Boolean).join(" | ")}` : ""}
            </p>
          ) : null}
        </div>
