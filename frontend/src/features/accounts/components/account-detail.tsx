    ...
    <div key={account.accountId} className="animate-fade-in-up space-y-4 rounded-xl border bg-card p-5">
      ...
      <div>
        ...
        <div className="mt-0.5 space-y-0.5 text-xs text-muted-foreground">
          ...
          {showUpstreamId ? <p title={`Upstream ${account.upstreamAccountId}`}>Upstream {compactUpstreamId}</p> : null}
          {showAccountId ? <p title={`Internal ${account.accountId}`}>Internal {compactId}</p> : null}
        </div>
      </div>
