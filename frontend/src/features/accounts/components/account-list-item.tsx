...
export function AccountListItem({ account, selected, showAccountId = false, onSelect }: AccountListItemProps) {
  ...
  const compactInternalId = formatCompactAccountId(account.accountId);
  const compactUpstreamId = account.upstreamAccountId ? formatCompactAccountId(account.upstreamAccountId) : null;
  const showUpstreamId = !!compactUpstreamId;
  const idSuffix = showAccountId ? ` | Internal ${compactInternalId}` : "";
  const metaParts = [
    baseSubtitle,
    showUpstreamId ? `Upstream ${compactUpstreamId}` : null,
    showAccountId ? `Internal ${compactInternalId}` : null,
  ].filter(Boolean);
      ...
      <div className="flex items-center gap-2.5">
        <div className="min-w-0 flex-1">
          ...
          <p
            ...
            title={[
              showUpstreamId ? `Upstream ${account.upstreamAccountId}` : null,
              showAccountId ? `Internal ${account.accountId}` : null,
            ].filter(Boolean).join(" | ") || undefined}
          >
            {emailSubtitle ? <><span className={blurred ? "privacy-blur" : undefined}>{emailSubtitle}</span>{showUpstreamId || showAccountId ? ` | ${[
              showUpstreamId ? `Upstream ${compactUpstreamId}` : null,
              showAccountId ? `Internal ${compactInternalId}` : null,
            ].filter(Boolean).join(" | ")}` : ""}</> : <>{metaParts.join(" | ")}</>}
          </p>
        </div>
