...
async def quota_shop_catalog(
    ...
    for option in options:
        ...
        if limit_type not in PURCHASABLE_TYPES:
            continue
        existing = next((x for x in summary.limits if x.limit_type == limit_type.value and x.limit_window == limit_window.value and x.model_filter == option_model), None)
        if existing is None and limit_window == LimitWindow.WEEKLY and limit_type == LimitType.COST_USD:
            existing = next((x for x in summary.limits if x.limit_type == limit_type.value and x.limit_window == limit_window.value), None)
        unit_size, unit_label = _rule_unit(limit_type)
        current = max(0, min(existing.current_value, existing.max_value)) if existing else 0
        rules.append(QuotaShopRule(
    ...
    async with get_background_session() as session:
        ...
        if (
            ...
            if limit is None and api_key.quota_shop_enabled:
                ...
                existing_windows = await session.execute(select(ApiKeyLimit.id).where(ApiKeyLimit.api_key_id == api_key.id))
                ...
                limit = ApiKeyLimit(api_key_id=api_key.id, limit_type=limit_type, limit_window=limit_window, max_value=0, current_value=0, model_filter=payload.model_filter, reset_at=expected_reset_at)
                session.add(limit)
                await session.flush()
            if limit is None:
