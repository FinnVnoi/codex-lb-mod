...
class RequestLogsService:
    ...
    async def list_recent(
        ...
        model_source_ids = [log.model_source_id for log in logs if log.model_source_id]
        api_key_name_by_id = await self._repo.get_api_key_names_by_ids(api_key_ids)
        model_source_name_by_id = await self._repo.get_model_source_names_by_ids(model_source_ids)
        requests = [
            to_request_log_entry(
                log,
