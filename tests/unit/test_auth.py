...
from app.core.auth import claims_from_auth, extract_id_token_claims, parse_auth_json, resolve_chatgpt_account_id
...
def test_claims_from_auth_prefers_explicit_workspace_id_over_account_id():
    payload = {
        "email": "user@example.com",
        "chatgpt_account_id": "acc_payload",
        "https://api.openai.com/auth": {"chatgpt_plan_type": "plus"},
    ...
    auth = parse_auth_json(json.dumps(auth_json).encode("utf-8"))
    claims = claims_from_auth(auth)
    assert claims.account_id == "ws_explicit"


def test_resolve_chatgpt_account_id_prefers_selected_organization():
    payload = {
        "email": "user@example.com",
        "chatgpt_account_id": "acc_payload",
        "organizations": [
            {"id": "org_other"},
            {"id": "org_selected", "selected": True},
        ],
    }
    token = _encode_jwt(payload)
    claims = extract_id_token_claims(token)
    assert resolve_chatgpt_account_id(claims=claims) == "org_selected"
