...
def _make_auth_json(
    ...
    *,
    ...
    doc: dict[str, object] = {"tokens": tokens}
    if workspace_id:
...
@pytest.mark.asyncio
async def test_import_supports_same_account_across_multiple_explicit_workspaces(async_client):
    settings = await async_client.put(
        "/api/settings",
        json={
            "stickyThreadsEnabled": False,
    ...
    email = "workspace-multi@example.com"
    raw_account_id = "acc_workspace_multi"
    ...
    first = await async_client.post(
        ...
        files={
            "auth_json": (
                "auth.json",
                json.dumps(_make_auth_json(raw_account_id, email, "team", workspace_id="ws_alpha")),
                "application/json",
            )
        },
    )
    second = await async_client.post(
        ...
        files={
            "auth_json": (
                "auth.json",
                json.dumps(_make_auth_json(raw_account_id, email, "team", workspace_id="ws_beta")),
                "application/json",
            )
        },
    )
    ...
    assert first.status_code == 200
    assert second.status_code == 200
    assert first.json()["accountId"] == generate_unique_account_id("ws_alpha", email)
    assert second.json()["accountId"] == generate_unique_account_id("ws_beta", email)
    ...
    ids = {entry["accountId"] for entry in accounts}
    assert ids == {
        generate_unique_account_id("ws_alpha", email),
        generate_unique_account_id("ws_beta", email),
    }
